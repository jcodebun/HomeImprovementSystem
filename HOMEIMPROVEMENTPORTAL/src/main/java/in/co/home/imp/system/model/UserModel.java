...
import in.co.home.imp.system.exception.RecordNotFoundException;
...
public class UserModel {
	...
	public Integer nextUID() throws DatabaseException {
		log.debug("Model nextPK Started");
		Connection conn = null;
		int pk = 0;
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(HomeId) FROM H_USER");
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				pk = rs.getInt(1);
			}
	...
	public void update(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					"UPDATE H_USER SET FIRSTNAME=?,LASTNAME=?,LOGIN=?,PASSWORD=?,MOBILENO=?,ROLEID=?,HomeId=?,"
					
					+ "CREATEDBY=?,MODIFIEDBY=?,CREATEDDATETIME=?,MODIFIEDDATETIME=?,Address=? WHERE ID=?");
			...
			pstmt.setLong(7,bean.getHomeId());
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getHomeId() > 0) {
				sql.append(" AND HomeId = " + bean.getRoleId());
			}
